...
func (c *ImageClient) BuildAndArchiveImage(ctx context.Context, outputLogger *slog.Logger, request *types.ContainerRequest) error {
	...
	// For built images, always use v1 (data extraction) since the image is already local
	// v2 is designed for remote registry images, not local builds
...
func (c *ImageClient) PullAndArchiveImage(ctx context.Context, outputLogger *slog.Logger, request *types.ContainerRequest) error {
	...
	if c.config.ImageService.ClipVersion == 2 {
		...
		// ✅ CRITICAL FIX: Index directly from the source registry reference
		// Do NOT use skopeo copy or local OCI directories!
		...
		if err != nil {
			...
			// v2 succeeded - push the metadata-only archive and return
			...
			log.Info().Str("image_id", request.ImageId).Dur("duration", elapsed).Msg("v2 archive created directly from registry")
			
			// Record metrics - v2 archives are tiny (metadata only)
			archiveInfo, _ := os.Stat(archivePath)
			if archiveInfo != nil {
				archiveSizeMB := float64(archiveInfo.Size()) / 1024 / 1024
				metrics.RecordImageCopySpeed(archiveSizeMB, elapsed)
			}
	...
	outputLogger.Info(fmt.Sprintf("Creating legacy archive (Clip v1, size: %.2f MB)...\n", imageSizeMB))
